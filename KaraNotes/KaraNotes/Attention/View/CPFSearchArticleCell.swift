...
class CPFSearchArticleCell: UITableViewCell {
    ...
    var searchArticleModel:CPFSearchArticleModel! {
        didSet {
            
            Alamofire.request(searchArticleModel.article_show_img_URL).responseImage { (response) in
                if let image = response.result.value {
                    self.thumbImage = image
                    self.thumbImageView.image = image
                }
            }
            titleLabel.text = searchArticleModel.article_title
            createTimeLabel.text = searchArticleModel.article_create_formatTime
        }
    }
...
extension CPFSearchArticleCell {
